...
//代码基于docx4j-3.2.0  
public class Docx4j_S3_Test {  

	public static void main(String[] args) throws Exception {
		baogaoyanzheng bgyz = new baogaoyanzheng();
		Docx4j_S3_Test dss = new Docx4j_S3_Test();
		String path ="f:/23.docx";
		WordprocessingMLPackage wmp = dss.loadWordprocessingMLPackage(path);
		List<Tbl> tblList = dss.getAllTbl(wmp);
		List<String> bbb = new ArrayList<>();
		for(int i=0;i<tblList.size();i++){
			List<String> aaa = dss.getTblContentList(tblList.get(i));
			bbb.addAll(aaa);
		}
		for (int j=0;j<bbb.size();j++){
			System.out.println(j+"--------------"+bbb.get(j));
		}


		String chujudate = bbb.get(4);
...
//		String tttt = bbb.get(1).substring(bbb.get(1).indexOf("价值时点")+4,bbb.get(1).indexOf("的估价结果"));
//		bgyz.setBgnum(bbb.get(3).substring(13,26));
//		String address = bbb.get(4).substring(bbb.get(4).indexOf("位于")+2,bbb.get(4).indexOf("成套住宅"));
//		bgyz.setBgname(address);
//		String pgjg = bbb.get(6).substring(bbb.get(6).indexOf(":")+1,bbb.get(6).length()-1);
//		System.out.println(bgyz.getBgnum());
...
//		System.out.println(tttt);
	...
	*/  
	 ...
	 * @Description: 设置字体信息
	 */
	public void setFontStyle(RPr runProperties, String cnFontFamily, String enFontFamily, String fontSize,
			String color) {
